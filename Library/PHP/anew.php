...
<html>
  ...
  <header class="headermain">
    ...
    <?php
      ...
      echo  '<div>','<form class="" action="anew.php" method="post" name="mngupdate">',
      '<button class="upapply" type="submit" name="button">Login</button>',
      '<input class="userenter" type="text" name="username" value="',$username,'" autocomplete="on"><br>','</form>','</div>',
    ...
    <ul>
      ...
      <li class="notactivepoint"><a href="add.php">Add univer ➲</a></li>
      <li class="activepoint"><a href="anew.php">Anew univer ➲</a></li>
      </div>
    </ul>
      </nav>
<div class="banner">
...
<div class="maindivtwo">
  ...
  <form class="" action="scripts/update.php" method="post" name="add">
    <label for="name">Please chose name university</label>
    <select class="" name="name">
    <option value="medical" selected >BGMU</option>
    <option value="dental">VNMU</option>
    <option value="engineer">UMLS</option>
    <option value="pharmacy">VGMU</option>
    <option value="pharmacy">MNU</option>
    <option value="pharmacy">Nation medicine</option>
    </select><br>
...
<?php/*
  ...
  $res->data_seek(0);
  while ($row = $res->fetch_assoc()) {
      echo " ➔ " .'<p class="into">', $row['id'],", "," art. - ",$row['art'],", "," country - ",$row['country'],", ", " type - ", $row['city'],", "," type - ",$row['type'],", "," name - ",$row['name'],", ", "address -", $row['address'],", "," website. - ",$row['website'],", "," info - ",$row['info'],", ","subsidiary -", $row['subsidiary'],", "," addresssub. - ",$row['addresssub'],", "," email - ", $row['email'],", ", " phone - ",$row['phone'],", "," manager of univerity - ",$row['manageruniver'],", "," medical - ",'<input type="checkbox" name="" disabled value=""',$row['medical'],'>',", "," dental - ",'<input type="checkbox" name="" disabled value=""',$row['dental'],'>',", ", " pharmacy - ", '<input type="checkbox" name="" disabled value=""',$row['pharmacy'],'>'," engineer - ",'<input type="checkbox" name="" disabled value=""',$row['engineer'],'>',"medical fee -", $row['medicalfee'],", "," dental fee - ",$row['dentalfee'],"pharmacy fee -", $row['pharmacyfee'],", "," engineer fee - ",$row['engineerfee'],", "," teach on english - ", $row['teacheng'],", ", " step - ",$row['step']," last answer - ",$row['lastanswer'],", "," conditions - ", $row['conditions'],", ", " status - ",$row['status']," history - ", $row['history'],", ", " other - ",$row['other']," reserve - ",$row['reserve'],'</p>', '<hr>' . "\n";
  }
  $mysqli->close();
  */?>
</div>
